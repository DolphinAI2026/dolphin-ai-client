      ...
      <div class="header">
        <div>
          ...
          <div class="meta">
            ...
            <span>草稿 <code>{{ data.draft_id }}</code></span>
...
import AnalysisView from '@/components/preview/AnalysisView.vue'
...
async function load() {
  ...
  try {
    ...
    error.value = e?.response?.data?.detail?.errors?.[0]?.msg || e?.message || '加载草稿失败'
  } finally {
    loading.value = false
  }
}
