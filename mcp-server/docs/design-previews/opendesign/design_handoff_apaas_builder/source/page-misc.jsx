    ...
    <div className="login-page">
      ...
      <div className="login-card">
        <div className="login-brand">
          ...
          <div className="login-title">aPaaS Builder AI</div>
          <div className="login-subtitle">得帆云低代码平台的智能搭建助手</div>
        </div>
      ...
      <div className="login-features">
        <div className="login-feature">
          <I.sparkle size={16} />
          <div>
            <div className="login-feature-title">AI 对话生成</div>
        ...
        <div className="login-feature">
          <I.store size={16} />
          <div>
            <div className="login-feature-title">组件市场</div>
            <div className="login-feature-desc">复用沉淀，团队协作</div>
          </div>
        </div>
      </div>
    </div>
...
function Admin() {
  const { adminTenants } = window.MOCK;
      ...
      <div className="page-pad">
        <div className="page-head">
          <div>
            ...
            <div className="page-subtitle">跨租户的资源与配置管理。</div>
          </div>
          <button className="btn btn-primary"><I.plus size={13} /> 新建租户</button>
        </div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="apps-row apps-row-head">
            <div style={{ flex: 2 }}>租户</div>
            <div style={{ width: 100 }}>套餐</div>
            <div style={{ width: 100 }}>状态</div>
            <div style={{ width: 120 }}>用户数 / 应用数</div>
            <div style={{ width: 140 }}>到期</div>
            <div style={{ width: 80 }} />
          </div>
          {adminTenants.map(t => (
            <div key={t.id} className="apps-row apps-row-item" style={{ cursor: 'default' }}>
              <div style={{ flex: 2, minWidth: 0 }}>
                <div className="apps-row-name">{t.name}</div>
                <div className="apps-row-sub mono">{t.code}</div>
              </div>
              <div style={{ width: 100 }}>
                <span className={`badge ${t.plan === '企业' ? 'badge-brand' : t.plan === '试用' ? 'badge-amber' : ''}`}>{t.plan}</span>
              </div>
              <div style={{ width: 100 }}>
                <span className={`badge ${t.status === 'active' ? 'badge-emerald' : 'badge-amber'}`}>
                  <span className="badge-dot" /> {t.status === 'active' ? '已启用' : '试用中'}
                </span>
              </div>
              <div style={{ width: 120, fontSize: 12.5, color: 'var(--text-2)' }} className="mono">
                {t.users} 用户 · {t.apps} 应用
              </div>
              <div style={{ width: 140, fontSize: 12.5, color: 'var(--text-3)' }}>{t.expiry}</div>
              <div style={{ width: 80, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                <button className="icon-btn" style={{ width: 26, height: 26 }}><I.gear size={13} /></button>
        ...
        <div className="card card-pad">
          {[
            { time: '14:23', who: '得帆云示例租户 · marshub', what: '部署应用「资产管理系统」到生产环境', tone: 'emerald' },
            { time: '13:50', who: '某汽车制造客户 · 张工', what: '新增模型配置（Qwen-Max）', tone: 'brand' },
            { time: '11:02', who: '某物流客户（试用） · admin', what: '试用账号即将到期（剩余 28 天）', tone: 'amber' },
            { time: '10:14', who: '得帆云示例租户 · 李宁', what: '发布组件「工单 SLA 看板」到组件市场', tone: 'sky' },
          ].map((a, i) => (
        ...
        <div className="page-head">
          <div>
            ...
            <div className="page-subtitle">
              Model Context Protocol 服务器接入。挂载后可在「智能对话」「AI Coding」「Vibe Coding」中被 AI 调用。
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div className="mcp-detail card">
              ...
              <div className="mcp-detail-section">
                ...
                <div className="mcp-scope-row">
                  <span className="mcp-scope-chip mcp-scope-on"><I.check size={11} /> AI 对话（智能搭建）</span>
                  <span className="mcp-scope-chip mcp-scope-on"><I.check size={11} /> 睿鲸 AI Coding</span>
                  <span className="mcp-scope-chip mcp-scope-off">Vibe Coding 全代码</span>
                </div>
              </div>
            </div>
          )}
