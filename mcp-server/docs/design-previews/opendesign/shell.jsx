...
const NAV = [
  { group: '睿鲸AI', items: [
    ...
    { key: 'apps',     label: '我的应用',       icon: 'apps',   path: '/apps', badge: 6 },
    { key: 'builder',  label: 'AI Builder',     icon: 'sparkle', path: '/ai-chat' },
  ]},
  { group: '开发扩展', items: [
    { key: 'coding',     label: 'AI Coding',       icon: 'whale', path: '/coding', badge: 1 },
  ...
  return (
    <aside className="rail">
      <div className="rail-brand">
        <button className="rail-logo" onClick={() => navigate('/')} aria-label="睿鲸AI">
        ...
        {!collapsed && (
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="rail-title">睿鲸AI</div>
          </div>
        )}
      ...
      <div className="rail-scroll">
        {NAV.map(grp => (
          <div className="rail-group" key={grp.group}>
            {!collapsed && grp.group !== '睿鲸AI' && <div className="rail-group-label">{grp.group}</div>}
            {grp.items.map(item => {
...
function TopBar({ crumb, actions, onCmdK }) {
  ...
  return (
    <div className="topbar">
      ...
      <button className="topbar-proj" onClick={() => navigate('/admin')} title="当前租户与平台管理">
        <span className="topbar-proj-stripe proj-stripe-emerald" />
        <span className="topbar-proj-name">Default Tenant</span>
        <span className="badge badge-emerald" style={{ marginLeft: 6 }}><span className="badge-dot" /> 已连接</span>
      </button>
      <button className="topbar-search" onClick={onCmdK}>
        <I.search size={14} />
        <span style={{ flex: 1, textAlign: 'left' }}>搜索应用、对话、设计文档…</span>
...
const CMDK = [
  { group: '导航', items: [
    { label: '首页', icon: 'home', path: '/', meta: 'G H' },
    { label: '我的应用', icon: 'apps', path: '/apps', meta: 'G A' },
    { label: 'AI Builder', icon: 'sparkle', path: '/ai-chat', meta: 'G B' },
    { label: 'AI Coding', icon: 'whale', path: '/coding' },
    { label: '组件市场', icon: 'store', path: '/marketplace' },
    { label: '平台管理', icon: 'admin', path: '/admin' },
  ]},
  { group: '快捷操作', items: [
    { label: '新建 AI Builder 对话', icon: 'plus', path: '/ai-chat?mode=requirements', meta: 'N C' },
    { label: '上传设计文档', icon: 'upload', path: '/' },
    { label: '生成低代码组件（AI Coding）', icon: 'whale', path: '/coding' },
    { label: '进入平台管理', icon: 'admin', path: '/admin' },
