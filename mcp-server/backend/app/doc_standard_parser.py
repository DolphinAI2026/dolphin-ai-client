...
def parse(text: str) -> ParseResult:
    ...
    if "roles" in sections:
        ...
        if not roles:
            result.errors.append("角色列表为空或不可解析，已按无角色清单处理；权限可使用“全部人员”")
            logger.info("角色列表解析为空，按可选模块跳过")
    else:
        result.errors.append('缺少"二、角色列表"章节，已按无角色清单处理；权限可使用“全部人员”')
    ...
    if "workflows" in sections:
        workflows, errs = workflows_parser.parse(sections["workflows"])
        result.errors.extend(errs)
    ...
    if "permissions" in sections:
        ...
        result.errors.append('缺少"权限配置/权限定义"章节')
        result.failed_modules.add("permissions")
