...
class CodingConversationCreate(BaseModel):
    title: Optional[str] = None
    selected_llm_config_id: Optional[int] = None


@router.post("/conversations")
async def create_coding_conversation(
    payload: CodingConversationCreate,
    ctx: Annotated[AuthContext, Depends(get_auth_context)],
    db: Annotated[AsyncSession, Depends(get_db)],
):
    """创建一个空的 Coding 会话，用于左侧历史列表和新会话占位。"""
    conversation = Conversation(
        title=(payload.title or "新会话")[:200],
        user_id=ctx.user.id,
        tenant_id=ctx.tenant_id,
        agent_type="coding",
        selected_llm_config_id=payload.selected_llm_config_id,
    )
    db.add(conversation)
    await db.commit()
