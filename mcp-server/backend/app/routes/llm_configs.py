...
async def list_llm_configs(
    ...
    result = await db.execute(
        select(LLMConfig)
        .where(LLMConfig.tenant_id.is_(None))
...
async def update_llm_config(
    ...
    """编辑平台全局 LLM 配置（管理员）"""
    result = await db.execute(
        select(LLMConfig).where(LLMConfig.id == config_id, LLMConfig.tenant_id.is_(None))
    )
...
async def get_llm_config_for_purpose(db: AsyncSession, tenant_id: int, purpose: str) -> Optional[LLMConfig]:
    """获取指定用途的全局默认 LLM 配置（供其他模块使用）。tenant_id 仅保留兼容调用签名。"""
    # 先找精确匹配的 purpose
    result = await db.execute(
...
async def list_llm_configs_for_purpose(db: AsyncSession, tenant_id: int, purpose: str) -> list[LLMConfig]:
    """列出指定用途可用的全局 LLM 配置，精确用途优先，其次 purpose=all。"""
    result = await db.execute(
        select(LLMConfig).where(
            LLMConfig.tenant_id.is_(None),
            LLMConfig.status == "active",
...
async def get_active_llm_config_by_id(
    ...
    """按 id 查 active 全局 LLM 配置；不限 purpose。
