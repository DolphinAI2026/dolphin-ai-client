...
- "直属上级"、"部门主管"、"部门经理"、"上级领导" 等层级关系角色**不需要创建**，平台直接读取组织架构，在审批流程节点中配置即可
- "sys_admin"、"系统管理员" 这类泛化平台管理员不要直接生成；需要管理员时生成带应用/业务语义的应用管理员，例如 meeting_admin、meet_mgmt_admin
- 人员、部门来自平台内置组织架构；业务表单需要部门归属时添加"所属部门"字段（部门选择），需要发起/参与人员时使用人员选择字段，不要生成员工表或部门表
...
- **【禁止】** 创建"直属上级"、"部门主管"、"部门经理"等层级角色，层级关系由平台组织架构自动处理
- **【禁止】** 创建 sys_admin 这种泛化管理员角色；应用管理员编码必须带业务语义或应用前缀
