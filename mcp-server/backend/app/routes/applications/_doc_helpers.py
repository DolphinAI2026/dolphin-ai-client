...
| ② 角色 | roles | 排除"全部员工/普通员工/部门主管/直属上级"等平台内置组织角色，只输出应用级管理员、会议室管理员、HR专员等有业务权限边界的角色 |
...
- **人员/部门来自平台组织架构**：不要把员工、普通员工、部门、部门主管、直属上级建成角色或数据模型；业务表单需要记录组织归属时，添加"所属部门"字段并使用平台部门选择。
...
- roles 只生成业务权限角色；不要生成 employee、dept_supervisor、direct_manager、sys_admin。需要应用管理员时，角色编码必须带应用语义，例如 `{app_code}_admin` 或 `meeting_admin`，角色名称用"XX管理员"。
- 审批中的部门负责人、直属上级只写在 flows.steps.role 作为平台组织审批规则，例如"发起人所属部门负责人"，不要放进 roles。
    ...
    repair_messages = [
        {
            ...
            "content": (
                ...
                "roles 只保留应用业务权限角色；不要生成普通员工、部门主管、直属上级、sys_admin。"
                "需要管理员时使用带应用语义的编码，例如 meeting_admin。"
...
PLATFORM_ALL_ROLE_CODES = {"all", "all_user", "all_employee", "employee", "staff", "user"}
PLATFORM_APPROVER_ROLE_CODES = {
    "approver",
    "leader",
    "supervisor",
    "dept_supervisor",
...
def _app_role_prefix(app_info: dict[str, str]) -> str:
    app_code = str(app_info.get("code") or "").strip() or _infer_lowcode_app_code(str(app_info.get("name") or ""))
    prefix = re.sub(r"[^a-z0-9_]", "_", app_code.lower())
    prefix = re.sub(r"_+", "_", prefix).strip("_")
    return prefix or "app"


def _app_admin_role(app_info: dict[str, str]) -> dict[str, str]:
    app_name = str(app_info.get("name") or "").strip() or "应用"
    return {
        "role_code": f"{_app_role_prefix(app_info)}_admin",
        "role_name": f"{app_name}管理员",
        "description": f"管理{app_name}的业务配置、全量数据和异常处理",
    }
...
def _default_business_fields(table_name: str) -> list[dict]:
    ...
    if "会议" in table_name:
        return [
            ...
            _field_template("initiator_name", "发起人", required=True),
            _field_template("department_name", "所属部门", required=True),
...
def _normalize_doc_roles(raw_roles: Any, app_info: dict[str, str]) -> list[dict[str, str]]:
    roles: list[dict[str, str]] = []
    seen: set[str] = set()
    app_admin = _app_admin_role(app_info)
    for idx, raw_role in enumerate(_list_or_empty(raw_roles)):
        ...
        if code_lower in GENERIC_ADMIN_ROLE_CODES or name in {"系统管理员", "管理员", "应用管理员", "业务管理员"}:
            code = app_admin["role_code"]
            name = app_admin["role_name"]
            code_lower = code.lower()
...
def _permissions_reference_generic_admin(raw_mappings: Any) -> bool:
    for raw_mapping in _list_or_empty(raw_mappings):
        ...
        for raw_perm in _list_or_empty(raw_mapping.get("permissions") or raw_mapping.get("rules")):
            if not isinstance(raw_perm, dict):
                continue
            role_code = str(raw_perm.get("role_code") or raw_perm.get("role") or "").strip().lower()
            role_name = str(raw_perm.get("role_name") or raw_perm.get("name") or "").strip()
            if role_code in GENERIC_ADMIN_ROLE_CODES or role_name in {"系统管理员", "管理员", "应用管理员", "业务管理员"}:
                return True
    return False


def _ensure_app_admin_role(roles: list[dict[str, str]], app_info: dict[str, str]) -> list[dict[str, str]]:
    app_admin = _app_admin_role(app_info)
    if any(role.get("role_code") == app_admin["role_code"] for role in roles):
        return roles
    return [app_admin] + roles
...
def _primary_business_role_name(roles: list[dict[str, str]]) -> str:
    return roles[0]["role_name"] if roles else "应用管理员"


def _normalize_flow_actor(value: Any, roles: list[dict[str, str]]) -> str:
    actor = str(value or "").strip()
    actor_lower = actor.lower()
    if not actor:
        return ""
    if actor_lower in PLATFORM_ALL_ROLE_CODES or actor in {"员工", "普通员工", "全体员工", "全部员工", "所有员工"}:
        return "全体人员"
    if actor_lower in PLATFORM_APPROVER_ROLE_CODES or actor in {"部门主管", "部门经理", "直属上级", "上级领导"}:
        return "发起人所属部门负责人"
    if actor_lower in GENERIC_ADMIN_ROLE_CODES or actor in {"系统管理员", "管理员", "应用管理员", "业务管理员"}:
        return _primary_business_role_name(roles)
    return actor
...
def _normalize_doc_flows(raw_flows: Any, tables: list[dict], roles: list[dict[str, str]]) -> list[dict]:
    ...
    for idx, raw_flow in enumerate(_list_or_empty(raw_flows)):
        ...
        for step_idx, raw_step in enumerate(_list_or_empty(raw_flow.get("steps") or raw_flow.get("nodes"))):
            ...
            steps.append({
                ...
                "role": _normalize_flow_actor(raw_step.get("role") or raw_step.get("assignee"), roles),
    ...
    for table in tables:
        ...
        flows.append({
            ...
            "steps": [
                {"step": 1, "action": f"提交{table['table_name']}", "role": "全体人员", "status": "待处理"},
...
def _normalize_doc_permissions(raw_mappings: Any, tables: list[dict], roles: list[dict[str, str]]) -> list[dict]:
    ...
    generic_admin_code = roles[0]["role_code"] if roles else ""

    def normalize_permission_role(raw_code: Any, raw_name: Any) -> tuple[str, str] | None:
        role_code = str(raw_code or "").strip()
        role_name = str(raw_name or role_code).strip()
        role_code_lower = role_code.lower()
        if role_code_lower in PLATFORM_ALL_ROLE_CODES or role_name in {"员工", "普通员工", "全体员工", "全部员工", "所有员工", "全部人员", "全体人员"}:
            return "all_employee", "全部人员"
        if role_code_lower in PLATFORM_APPROVER_ROLE_CODES or role_name in {"部门主管", "部门经理", "直属上级", "上级领导"}:
            return None
        if role_code_lower in GENERIC_ADMIN_ROLE_CODES or role_name in {"系统管理员", "管理员", "应用管理员", "业务管理员"}:
            if not generic_admin_code:
                return None
            role = role_by_code.get(generic_admin_code)
            return generic_admin_code, role["role_name"] if role else "应用管理员"
        if role_code and role_code in role_by_code:
    ...
    for raw_mapping in _list_or_empty(raw_mappings):
        ...
        for raw_perm in _list_or_empty(raw_mapping.get("permissions") or raw_mapping.get("rules")):
            ...
            normalized_role = normalize_permission_role(role_code, role_name)
            if not normalized_role:
                continue
            role_code, role_name = normalized_role
...
def normalize_doc_result(doc: dict, messages: list[dict[str, Any]] | None = None) -> dict:
    ...
    roles = _normalize_doc_roles(source.get("roles"), app_info)
    raw_permissions = source.get("role_table_mapping") or source.get("permissions")
    if _permissions_reference_generic_admin(raw_permissions):
        roles = _ensure_app_admin_role(roles, app_info)
...
def json_to_markdown(data: dict) -> str:
    ...
    def component_label(value: Any, field: dict | None = None) -> str:
        ...
        if "部门" in field_name:
            return "部门选择"
        if any(key in field_name for key in ("发起人", "申请人", "经办人", "负责人", "参会人")):
            return "人员选择"
