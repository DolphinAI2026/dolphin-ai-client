...
async def run_coding_pipeline(
    ...
    try:
        ...
        elif coding_conversation:
            if params.selected_model is not None:
                coding_conversation.selected_llm_config_id = effective_model_config_id
            if (coding_conversation.title or "").strip() in ("新会话", "未命名会话"):
                coding_conversation.title = params.message[:50]
            await db.commit()
            await db.refresh(coding_conversation)
