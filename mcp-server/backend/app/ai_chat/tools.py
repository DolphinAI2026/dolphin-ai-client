...
TOOL_SCHEMAS: list[dict] = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "如果是要创建/预览 aPaaS Builder 应用的标准设计文档，优先直接调用 save_design_draft，"
                "它会同时保存 Markdown 到右侧设计文档面板并返回 HTML 效果预览链接。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "向用户提一个澄清问题，并提供候选答案。耗材管理这类常见系统不要问主流程，AI 自己梳理。"
                "最多 3 轮，顺序固定：1 问模块范围；2 问审批/流转；3 问权限分配。"
                "模块范围示例：耗材档案、入库、领用/出库、库存台账、盘点、预警、采购、供应商、报废/归还、统计。"
                "审批流示例：领用是否审批、入库是否仓管确认、盘点差异/报废是否审批。"
                "权限示例：应用管理员、仓管等业务角色怎么分权；全体人员、本部门等平台范围怎么看。"
                "用户回答过模块、审批流、权限后，禁止继续调用本工具，必须生成完整设计文档并调用 save_design_draft。"
...
async def execute_ask_clarifying_question(
    ...
    text = question + " " + " ".join(str(x) for x in (options if isinstance(options, list) else []))
    # Guardrail: consumables should ask product-design decisions, not force the
    # user to choose a business mainline the AI can infer.
    has_consumable_context = "耗材" in text
    asks_mainline = re.search(r"主流程|业务主流程|流程怎么走|主链路|入库\s*→|库存台账\s*→|领用/出库", text)
    if has_consumable_context and asks_mainline:
        question = "这个耗材管理系统需要哪些模块？"
        options = [
...
async def execute_tool(
    ...
    if tool_name in list_mcp_tool_names_cached():
        ...
        # save_design_draft 也要落一份原始 md 到 artifact：跳过 write_artifact
        # 只是避免 LLM 重写，不代表前端右侧"设计文档"可以没有内容。
        if tool_name in ("generate_app_from_doc", "update_app_from_doc", "save_design_draft"):
...
async def _persist_spec_artifact(
    ...
    """把工具入参里的 md_content 落 artifact 表。
    ...
    existing = await db.execute(
        ...
        .where(
            ...
            AIChatArtifact.content == md_content,
        )
        .limit(1)
    )
    ...
    if not fname:
        ...
        try:
            ...
            if isinstance(parsed, dict):
                ...
                draft_id = (parsed.get("draft_id") or parsed.get("id") or "").strip()
                if rname:
                    fname = f"{rname}-设计文档.md"
                elif rid:
                    fname = f"app-{rid}-设计文档.md"
                elif draft_id:
                    fname = f"{draft_id}-设计文档.md"
        except Exception:
            pass

    if not fname:
        title_match = re.search(r"^\s*#\s+(.+?)\s*$", md_content, re.M)
        if title_match:
