...
class Settings(BaseSettings):
    ...
    # Design Preview（给 agent 返回的预览页 URL 基址）
    # 后端直出 HTML（无鉴权，靠 draft_id 不可枚举）
    # base 应指向 /api 路径（跟 nginx /mcp-server/api/ 反代对齐）
    # 本地：http://localhost:8004/api
    # 线上：https://agent.dfy.definesys.cn/mcp-server/api
    preview_base_url: str = "http://localhost:8004/api"
    ...
    dolphin_copilot_agent_code: str = ""  # 2026-05-06: AI-Builder 伴侣（统一 agent + 4 Skill：req-design/app-create/app-adjust/app-publish）
    # 2026-05-08: AI-aPaaS-Coding 低代码自开发 agent（dev-coding skill + 23 个 MCP 工具：
    # form-component-dual / form-page / backend-api 等场景，agent 自己拿 formId/tabId/uuid 写代码 build 部署）。
    # 跟 copilot_agent_code 平级，前端 NavRail "低代码自开发" 项条件显示。
    dolphin_coding_agent_code: str = ""
    ...
    # 2026-05-11 dolphin SDK 官方公开参数：按 agent 产品语义固定 instanceId（跨客户共享）
    # 平台管理后台只读显示；调整时改 .env
    dolphin_builder_instance_id: str = "ai-apaas-builder"  # copilot/app_adjust/requirements/default agent
    dolphin_coding_instance_id: str = "ai-apaas-coding"   # coding agent
    ...
    # AI-Builder 主服务 API 对外 URL。线上 agent.dfy 的 /ai-builder 指向 ming，
    # 主服务 API 产物下载需要走 df-aigc 的 /ai-builder/api。
    ai_builder_api_public_base: str = "https://df-aigc.dfy.definesys.cn"
