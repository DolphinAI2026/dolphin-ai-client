...
_ALL_USERS_TOKENS = {"全部人员", "全员", "所有人", "所有用户", "all", "*", "ALL_USER"}
...
def parse(
    ...
    for row in rows:
        ...
        role_code = (row.get("角色编码") or row.get("授权对象") or row.get("权限对象") or "").strip()
        ...
        if not role_code:
            role_code = "all"
        role_name = ""
        if role_code in _ALL_USERS_TOKENS:
            role_code = "all"
            role_name = "全部人员"
        if role_codes and role_code != "all" and role_code not in role_codes:
