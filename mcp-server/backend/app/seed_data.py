...
async def sync_builtin_llm_configs(
    ...
    *,
    ...
    """把 .env 中的内置模型同步到平台全局 llm_configs，避免前台重复手工配置。
    ...
    existing_global = (
        await db.execute(select(LLMConfig).where(LLMConfig.tenant_id.is_(None)))
    ).scalars().all()

    # 清理 obsolete builtin（不影响用户手工添加的同名 / 自定义命名 config）
    obsolete_rows = [
        row for row in existing_global
        if row.config_name in _OBSOLETE_BUILTIN_NAMES
    ]
    for row in obsolete_rows:
        await db.delete(row)
    await db.flush()

    existing_global = [row for row in existing_global if row not in obsolete_rows]
    if existing_global:
        if commit:
            await db.commit()
        else:
            await db.flush()
        print("✅ 已存在全局 LLM 配置，跳过内置 LLM 同步")
        return
