...
def _form_import_enabled(user_perm: Optional[dict]) -> bool:
    """设计文档权限里任一角色可导入，则需要开启表单 Excel 导入模板。"""
    if not user_perm:
        return False
    for rule in user_perm.get("rules") or []:
        if isinstance(rule, dict) and bool(rule.get("canImport")):
            return True
    return False


async def _enable_excel_import_for_forms(
    client: APaaSClient,
    app_id: str,
    jobs: List[dict],
) -> int:
    """按表单开启 Excel 导入模板；单表失败不阻断应用搭建。"""
    enabled_count = 0
    for job in jobs:
        form_id = job.get("form_id")
        if not form_id:
            continue
        try:
            await client.enable_excel_import_config(app_id, form_id)
            enabled_count += 1
            logger.info(
                "开启表单 Excel 导入模板成功: formId=%s formName=%s",
                form_id,
                job.get("form_name", ""),
            )
        except Exception as exc:
            logger.warning(
                "开启表单 Excel 导入模板失败（不阻断）: formId=%s formName=%s error=%s",
                form_id,
                job.get("form_name", ""),
                exc,
            )
        ...
        try:
            ...
            for fr in form_results:
                ...
                if built is not None:
                    ...
                    if _form_import_enabled(user_perm):
                        import_config_jobs.append(sync_job)
            ...
            if import_config_jobs:
                enabled_count = await _enable_excel_import_for_forms(client, app_id, import_config_jobs)
                if enabled_count:
                    yield {"stage": 4, "status": "running", "step": f"开启 {enabled_count} 个表单导入模板"}
