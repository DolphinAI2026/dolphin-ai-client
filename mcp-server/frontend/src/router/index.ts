...
const router = createRouter({
  ...
  routes: [
    ...
    },
    {
      ...
      name: 'Chat',
      redirect: to => ({
        path: to.params.id ? `/ai-chat/${to.params.id}` : '/ai-chat',
        query: to.query,
      }),
    },
    {
    ...
    },
    {
      path: '/work/:appId',
      name: 'WorkspaceShell',
      redirect: to => ({ path: '/ai-chat', query: { app_id: String(to.params.appId) } }),
      meta: { requiresAuth: true }
    },
    {
      path: '/platform-envs',
    ...
    },
    {
      ...
      redirect: to => ({
        path: '/ai-chat',
        query: {
          app_id: to.params.id as string,
          prompt: '请基于当前应用的设计文档创建并发布应用，严格走 save_design_draft/promote_draft_to_app 主流程。',
        },
      }),
      meta: { requiresAuth: true }
    }
...
router.afterEach((to) => {
  document.title = to.path.startsWith('/admin') || to.path.startsWith('/platform-admin')
    ? '睿鲸AI平台'
    : '睿鲸AI工作台'
})
