    ...
    <div class="coding-body">
      ...
      <div class="main-content">
        ...
        <div v-if="!isStreaming && streamMessages.length === 0 && !codingStore.workspace" class="welcome-pane">
          <div class="coding-chat-shell">
            <input
              ref="fileInputRef"
              type="file"
              accept=".zip,.md,.pdf,.docx,.txt,.png,.jpg,.jpeg"
              style="display: none"
              @change="handleFileSelect"
            />

            <header class="coding-chat-topbar">
              <strong>新会话</strong>
            </header>

            <main class="coding-chat-main">
              <div class="coding-chat-empty">
                <h1>开始对话</h1>
                <p>在下方输入组件、页面、接口或脚本需求。</p>
              </div>
            </main>

            <footer class="coding-chat-composer">
              <div class="coding-chat-composer-box" @paste="handlePaste">
                <div v-if="attachedFile" class="coding-chat-attach">
                  <div v-if="attachedPreviewUrl" class="coding-chat-attach-thumb">
                    <img :src="attachedPreviewUrl" alt="preview" />
                    <button type="button" class="coding-chat-attach-remove" @click="removeAttachment">&times;</button>
                  </div>
                  <div v-else class="coding-chat-attach-file">
                    <span class="coding-chat-attach-name">{{ attachedFile.name }}</span>
                    <button type="button" class="coding-chat-attach-remove" @click="removeAttachment">&times;</button>
                  </div>
                </div>

                <div class="coding-chat-input-row">
                  <button
                    type="button"
                    class="coding-chat-icon-btn"
                  ...
                  <textarea
                    ...
                    v-model="userInput"
                    class="coding-chat-input"
                    :placeholder="`描述要开发的${activeSceneCategoryLabel}，或要求修改已有组件`"
                    rows="1"
                ...
                <div class="coding-chat-toolbar">
                  ...
                  <span class="coding-chat-model-hint">首条消息将使用当前模型</span>
                </div>
              </div>
            </footer>
          </div>
...
import type { WorkspaceInfo, ReplayStreamMessage, CodingConversation } from '@/api/coding'
...
async function loadCodingConversations(activeConversationId?: number | null) {
  try {
    const list = await codingApi.getConversations()
    codingConversations.value = [...list].sort((a, b) =>
      new Date(b.updated_at || b.created_at || 0).getTime() - new Date(a.updated_at || a.created_at || 0).getTime()
    )
    if (activeConversationId) {
      codingStore.conversationId = activeConversationId
    }
  } catch (e) {
    console.warn('加载 AI Coding 会话历史失败:', e)
  }
}

const sidebarCodingItems = computed<SidebarSessionItem[]>(() =>
  (codingConversations.value || []).map((conv) => ({
    id: conv.id,
    title: conv.title || '新会话',
    meta: conv.workspace_id ? '已生成工作区' : '对话中',
    group: conversationGroupLabel(conv.updated_at || conv.created_at),
  }))
)
const sidebarCodingActiveId = computed<number | null>(() => codingStore.conversationId || null)

async function onSidebarCodingSelect(id: string | number) {
  const conversationId = Number(id)
  if (!Number.isFinite(conversationId)) return
  if (sidebarCodingActiveId.value === conversationId) return
  if (openingWsId.value) return
  openingWsId.value = String(conversationId)
  try {
    const target = codingConversations.value.find((item) => item.id === conversationId)
    if (target) {
      await openCodingConversation(target)
    }
  } finally {
...
async function onSidebarCodingCreate() {
  await startNewWorkspace({ createConversation: true })
}
...
onMounted(async () => {
  ...
  try {
    ...
    await loadCodingConversations()
  } catch (e) {
    console.error('\u521D\u59CB\u5316 AI Coding \u9875\u9762\u5931\u8D25:', e)
  }

  const wsId = (route.query.workspace_id || route.query.ws) as string
  const conversationId = Number(route.query.conversation_id || 0)
  if (Number.isFinite(conversationId) && conversationId > 0) {
    const target = codingConversations.value.find((item) => item.id === conversationId)
    if (target) await openCodingConversation(target)
  } else if (wsId) {
...
async function openCodingConversation(conv: CodingConversation) {
  try {
    codingStore.conversationId = conv.id
    applyCodingModelSelection(conv.selected_llm_config_id ?? null)
    ...
    const [messages, workspace, workspaceConversation] = await Promise.all([
      conv.workspace_id
        ? Promise.resolve([])
        : codingApi.getMessages(conv.id),
      conv.workspace_id ? codingApi.getWorkspace(conv.workspace_id).catch(() => null) : Promise.resolve(null),
      conv.workspace_id ? codingApi.getWorkspaceConversation(conv.workspace_id).catch(() => null) : Promise.resolve(null),
    ])

    if (workspace) {
      codingStore.setWorkspace(workspace)
      codingStore.conversationId = workspaceConversation?.conversation_id || conv.id
      localStorage.setItem('coding_last_workspace_id', workspace.id)
      try {
        const { ide_url } = await codingApi.getIdeUrl(workspace.id, conv.id, themeStore.mode)
      ...
      codingStore.workspace = null
      codingStore.workspacePath = null
    ...
    for (const key of ['workspace_id', 'ws', 'online_workspace_id', 'online_ws', 'online_view', 'from_ai_builder']) {
      delete query[key]
    }
    await router.replace({
      ...
      query: {
        ...
        ...(conv.workspace_id ? { workspace_id: conv.workspace_id } : {}),
      },
    })
  } catch (error: any) {
    ElMessage.error(`打开会话失败: ${error?.message || '请重试'}`)
  }
}
...
async function openWorkspaceById(wsId: string) {
  try {
    ...
    await loadCodingConversations(workspaceConversation.conversation_id)
...
async function startNewWorkspace(options: { createConversation?: boolean } = {}) {
  codingStore.reset()
  userInput.value = ''
  ...
  codingArtifactPanelUserToggle.value = null
  localStorage.removeItem('coding_last_workspace_id')

  const query = { ...route.query }
  let shouldReplaceRoute = false
  for (const key of ['workspace_id', 'ws', 'conversation_id', 'online_workspace_id', 'online_ws', 'online_view', 'from_ai_builder']) {
    if (key in query) {
      delete query[key]
      shouldReplaceRoute = true
    }
  }
  if (route.path !== '/coding') {
    await router.replace({ path: '/coding', query })
  } else if (shouldReplaceRoute) {
    await router.replace({ query })
  }

  if (options.createConversation) {
    try {
      const created = await codingApi.createConversation({
        title: '新会话',
        selected_llm_config_id: parseCodingModelConfigId(selectedCodingModelValue.value),
      })
      codingStore.conversationId = created.id
      applyCodingModelSelection(created.selected_llm_config_id ?? null)
      codingConversations.value = [
        created,
        ...codingConversations.value.filter((item) => item.id !== created.id),
      ]
      await router.replace({
        path: '/coding',
        query: {
...
const { sendMessage, sendSuggestion } = useCodingPipeline({
  ...
  onConversationUpdated: loadCodingConversations,
})
...
.coding-chat-composer {
  padding: 18px 28px 22px;
  border-top: 1px solid #e8edf4;
  background: rgba(247, 249, 252, 0.92);
}

.coding-chat-composer-box {
...
.coding-chat-composer-box:focus-within {
  border-color: #b9c8f7;
...
.coding-chat-icon-btn:hover:not(:disabled) {
  background: #f1f5fb;
  color: #66748f;
}
...
.coding-chat-send:not(:disabled):hover {
  background: #7f91ef;
}

.coding-chat-send:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
...
.coding-chat-model:hover:not(:disabled),
.coding-chat-model.is-open {
  background: #fff;
  border-color: #b9c8f7;
  color: #4f5fe0;
}

.coding-chat-model.is-disabled,
.coding-chat-icon-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.coding-chat-model-hint {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  color: #94a3b8;
...
.coding-chat-attach {
  padding: 10px 14px 0 52px;
}

.coding-chat-attach-thumb,
.coding-chat-attach-file {
  position: relative;
  display: inline-flex;
  align-items: center;
  gap: 8px;
...
.coding-chat-attach-remove:hover {
