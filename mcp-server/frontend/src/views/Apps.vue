...
/* ── Fix 15 (2026-05-21): 卡片视图 "更多" 菜单 ───────────────────────────
   ...
   - 查看 SPEC：跳 AIChat tab=spec（hasDesignOutput 才显示）
...
function openConversation(app: MergedApplication, item?: ConversationWithApp) {
  if (!item) return
  router.push({ path: '/ai-chat', query: { app_id: String(app.id), conversation_id: String(item.id) } })
}
