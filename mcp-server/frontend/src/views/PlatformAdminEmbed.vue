...
function handleAdminMessage(event: MessageEvent) {
  // 只接受来自 admin-spa iframe 的同 origin message
  // dev: frontend vite 通过 /admin 代理到 backend，prod: 同 origin /admin/
  ...
  if (event.data?.type === 'admin-return-workspace') {
    const target = typeof event.data?.targetUrl === 'string'
      ? event.data.targetUrl
      : `${import.meta.env.BASE_URL || '/'}`
    window.location.href = target
  } else if (event.data?.type === 'admin-logout') {
