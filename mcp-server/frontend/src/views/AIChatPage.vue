<template>
  ...
  >
    ...
    <main class="chat-main">
      <header class="chat-header">
        ...
        <div class="header-actions">
          ...
          >📄 设计文档 <span class="badge">1</span></button>
        </div>
      </header>
      ...
      >
        ...
        <!-- 应用就绪 CTA — promote_draft_to_app 成功后 inline 一张大卡，
             一键跳应用入口。
        ...
        <template #custom="{ message }">
          ...
          <div
            v-else-if="message.meta?.kind === 'draft-saved' && message.meta?.draft"
            class="draft-saved-card"
          >
            <div class="draft-card-icon">📋</div>
            <div class="draft-card-body">
              <div class="draft-card-title">设计文档已生成</div>
              <div class="draft-card-sub">
                <span>{{ message.meta.draft.summary || '可查看文档和预览效果，确认后创建应用' }}</span>
                <span v-if="message.meta.draft.draftId" class="draft-card-id">{{ message.meta.draft.draftId }}</span>
              </div>
            </div>
            <button
              v-if="designDocumentArtifact"
              class="draft-card-action secondary"
              type="button"
              @click="openLatestDesignDocument"
            >
              查看文档
            </button>
            <button
              v-if="message.meta.draft.previewUrl"
              class="draft-card-action"
              type="button"
              @click="openDraftPreview(message.meta.draft.previewUrl)"
            >
              预览效果
            </button>
          </div>
    ...
    <aside
      class="aside-right"
      v-if="currentSession && artifactsPanelOpen && designDocumentArtifact"
    ...
    >
      ...
      <div class="art-header">
        ...
        <span class="art-breadcrumb">
          ...
          <span class="sep">›</span>
          <span class="seg current">设计文档</span>
        </span>
      </div>
      <div class="art-preview" v-if="activeArtifactContent">
        <div class="art-preview-head">
          ...
          <button
            v-if="latestDesignPreviewUrl"
            class="small-btn"
            @click="openDraftPreview(latestDesignPreviewUrl)"
            title="打开 HTML 效果预览"
          >预览效果</button>
          <!-- 设计文档完成 → 一键让 agent 调 save_design_draft/promote_draft_to_app 创建应用。
          ...
          <button
            ...
            title="把当前设计文档发给 AI Builder，对话里创建应用"
          >→ 生成应用</button>
          <button
            v-if="canSendArtifactToBuilder && alreadyDeployedAppId"
            class="small-btn primary"
...
type TransientItem =
  ...
  | { kind: 'draft_card'; draft: { draftId: string; summary?: string; previewUrl?: string; tcId?: number }; ts: number }
...
const renderMd = (text: string): string => {
  if (!text) return ''
  try {
    return addBlankTargets(marked.parse(text) as string)
  } catch (e) {
...
function addBlankTargets(html: string): string {
...
function summarizeToolResult(name: string, status: string, resultText: string | null | undefined): string {
  ...
  if (name === 'save_design_draft') {
    if (r.ok === false) return '❌ 设计文档保存失败'
    const draftId = r.draft_id || r.id
    if (draftId) return `✅ 设计文档已生成 ${draftId}`
    return '✅ 设计文档已生成'
  }
  if (name === 'promote_draft_to_app') {
    ...
    if (appId && apaasId) return `✅ app_id=${appId} apaas_app_id=${apaasId}`
 ...
 *
 * 触发：tool_calls 数组里存在 promote_draft_to_app 状态为 success
 * 且 result JSON 含 app_id（或 apaas_app_id）。
 *
 * 选取规则：取最新一次（按 tool.id 倒序）匹配 tool 的 result 作为锚点。
 *
...
function normalizeGeneratedAppName(value: unknown): string {
  const raw = String(value || '').trim()
  if (!raw) return ''
  const cleaned = raw.replace(/^「|」$/g, '').trim()
  if (!cleaned || ['未命名应用', '业务应用', '应用', '应用设计文档'].includes(cleaned)) return ''
  return cleaned
}

function appNameFromDraftSummary(summary?: string): string {
  const first = String(summary || '').split('|')[0]?.trim() || ''
  return normalizeGeneratedAppName(first)
}

const latestDraftAppName = computed(() => {
  for (let i = toolCalls.value.length - 1; i >= 0; i--) {
    const tc = toolCalls.value[i]
    if (!tc || tc.tool_name !== 'save_design_draft' || tc.status !== 'success') continue
    const name = appNameFromDraftSummary(parseDraftFromResult(tc.result_text)?.summary)
    if (name) return name
  }
  return ''
})
...
const appReadyInfo = computed<AppReadyInfo | null>(() => {
  ...
  //   - promote_draft_to_app  → app_id / apaas_app_id / admin_url
  ...
  const TRIGGER_TOOLS = new Set(['promote_draft_to_app'])
  const MERGE_TOOLS = new Set(['promote_draft_to_app', 'get_application'])
  for (const tc of tcs) {
    if (!tc) continue
    ...
    if (r.app_name || r.application_name) appName = normalizeGeneratedAppName(r.app_name || r.application_name)
    if (r.app_code) appCode = String(r.app_code)
    if (r.app_view_url || r.admin_url) appViewUrl = String(r.app_view_url || r.admin_url)
  ...
  return {
    ...
    appId,
    apaasAppId,
    appName: appName || latestDraftAppName.value || '应用',
    appCode,
    appViewUrl,
  }
})
...
function openAppReady(info: AppReadyInfo) {
  ...
  // fallback：用 app_id 回到 AIChat 主线
  if (info.appId) {
    router.push({ path: '/ai-chat', query: { app_id: String(info.appId) } })
    return
  }
...
function openDraftPreview(url: string) {
  if (!url) return
...
function collapseTools(tcs: AIChatToolCall[], seenWriteForFile: Record<string, number>): TLItem[] {
  const out: TLItem[] = []
  const hasSavedDesignDoc = tcs.some(t => t.tool_name === 'save_design_draft' && t.status === 'success')
  const visibleTcs = hasSavedDesignDoc
    ? tcs.filter(t => !(t.tool_name === 'write_artifact' && t.status === 'success'))
    : tcs
  let i = 0
  while (i < visibleTcs.length) {
    let j = i + 1
    while (j < visibleTcs.length && visibleTcs[j].tool_name === visibleTcs[i].tool_name) j++
    if (j - i >= 2) {
      out.push({ kind: 'tool_group', tools: visibleTcs.slice(i, j) })
    } else {
      out.push({ kind: 'tool', tool: visibleTcs[i] })
    }
    // 如果当前段最后一条是 write_artifact 成功，紧跟一张 inline artifact card。
    // 但 write_artifact 后马上 save_design_draft 时，用户感知上应是一个“设计文档”
    // 工具入口：md 文档 + HTML 预览都放进后面的设计文档卡片，避免一屏两张卡。
    const last = visibleTcs[j - 1]
    if (last && last.tool_name === 'write_artifact' && last.status === 'success') {
      const fname = last.args_json?.filename
      const hasFollowingDesignDocSave = visibleTcs
        .slice(j)
        .some(t => t.tool_name === 'save_design_draft' && t.status === 'success')
      if (fname && !hasFollowingDesignDocSave) {
    ...
    if (last && last.tool_name === 'save_design_draft' && last.status === 'success') {
      const draft = parseDraftFromResult(last.result_text)
      if (draft) {
        out.push({ kind: 'draft_card', draft: { ...draft, tcId: last.id }, ts: 0 })
      }
    }
    i = j
  }
...
function parseDraftFromResult(result_text: string | null | undefined): { draftId: string; summary?: string; previewUrl?: string } | null {
  const parsed = _parseToolResult(result_text)
  if (!parsed || parsed.ok === false) return null
  const draftId = parsed.draft_id || parsed.id
  if (!draftId) return null
  return {
    draftId,
    summary: parsed.summary || undefined,
    previewUrl: parsed.preview_url || undefined,
  }
}
...
function cleanAssistantMessage(content: string): string {
  ...
  const kept: string[] = []
  let skippingDeployDetails = false
  for (const raw of lines) {
    const line = raw.trim()
    const normalized = line.replace(/^\s*[-*]\s*/, '')

    if (/^(文档编号|概要|预览链接)\s*[:：]/.test(normalized)) continue
    if (/^请先\s*review\s*右侧\s*Markdown\s*文档和预览效果/.test(normalized)) continue
    if (/开始创建|开始部署|创建\s*\/\s*部署|\/\s*OK|或\s*OK/i.test(normalized)) continue

    if (/^(app_id\s*\/\s*local_app_id|app_id|local_app_id|apaas_app_id)\s*[:：]/i.test(normalized)) {
      skippingDeployDetails = true
      continue
    }
    if (/^后台管理地址\s*[:：]?/.test(normalized)) {
      skippingDeployDetails = true
      continue
    }
    if (skippingDeployDetails && /^https?:\/\//i.test(normalized)) continue
    if (skippingDeployDetails && !normalized) {
      skippingDeployDetails = false
      continue
    }
    skippingDeployDetails = false
...
const agentMessages = computed<AgentMessage[]>(() => {
  ...
  let pendingDraftAsk: AgentMessage | null = null
  const flushPendingDraftAsk = () => {
    if (!pendingDraftAsk) return
    out.push(pendingDraftAsk)
    pendingDraftAsk = null
  }
  const mapStatus = (s: string): 'pending' | 'running' | 'success' | 'error' =>
    (s === 'aborted' ? 'error' : (s as any)) || 'pending'
  const displayToolName = (name: string) => (
    name === 'write_artifact' || name === 'save_design_draft'
      ? '设计文档'
      : name
  )
  const mapTool = (tc: AIChatToolCall) => ({
    id: tc.id,
    name: displayToolName(tc.tool_name),
    args: tc.args_json,
  ...
  const hasSavedDesignDoc = renderTimeline.value.some(item => item.kind === 'draft_card')
  for (const item of renderTimeline.value) {
    if (item.kind === 'msg' && item.msg.role === 'user') {
      ...
      const content = cleanAssistantMessage(item.msg.content)
      if (content) {
        out.push({ id: 'm' + item.msg.id, kind: 'assistant', content })
        flushPendingDraftAsk()
      }
      ...
      const answered = latestUserMessageAfterTool(item.ask.tc_id)
      out.push({
        id: 'ask' + item.ask.tc_id,
        kind: 'ask',
        ask: { question: item.ask.question, options: item.ask.options, answered },
      })
    } else if (item.kind === 'draft_card') {
      if (seenDraftIds.has(item.draft.draftId)) continue
      seenDraftIds.add(item.draft.draftId)
      out.push({
        id: 'draft' + item.draft.draftId,
        kind: 'custom',
        meta: { kind: 'draft-saved', draft: item.draft },
      })
      if (!appReadyInfo.value && item.draft.tcId) {
        const answered = latestUserMessageAfterTool(item.draft.tcId)
        pendingDraftAsk = {
          id: 'draft-action-' + item.draft.draftId,
          kind: 'ask',
          ask: {
            question: '确认文档和预览后，下一步要创建并部署应用吗？',
            options: ['开始创建并部署应用'],
            answered,
          },
        }
      ...
      if (hasSavedDesignDoc && isMarkdownArtifact(item.artifact)) continue
      if (seenArtifactIds.has(item.artifact.id)) continue
      seenArtifactIds.add(item.artifact.id)
      out.push({
      ...
      const content = cleanAssistantMessage(item.text)
      if (content) out.push({ id: 'streaming', kind: 'streaming', content, streaming: true })
    }
  }
  flushPendingDraftAsk()
  return out
...
function latestUserMessageAfterTool(toolCallId: number): string {
  const tc = toolCalls.value.find(t => t.id === toolCallId)
  const tcTs = tsOf(tc?.ended_at || tc?.started_at)
  if (!tcTs) return ''
  const laterUser = messages.value
    .filter(m => m.role === 'user' && tsOf(m.created_at) > tcTs)
    .sort((a, b) => tsOf(b.created_at) - tsOf(a.created_at))[0]
  return laterUser?.content || ''
}
...
async function openLatestDesignDocument() {
  artifactsPanelOpen.value = true
  const latest = designDocumentArtifact.value
  if (latest) await loadArtifact(latest)
}

const designDocumentArtifact = computed<AIChatArtifact | null>(() => {
  const mdArtifacts = artifacts.value.filter(a => isMarkdownArtifact(a))
  if (!mdArtifacts.length) return null
  return [...mdArtifacts].sort((x, y) => {
    const tx = new Date(x.created_at || x.updated_at || 0).getTime()
    const ty = new Date(y.created_at || y.updated_at || 0).getTime()
    if (ty !== tx) return ty - tx
    return (y.version || 0) - (x.version || 0)
  })[0]
})

const latestDesignPreviewUrl = computed(() => {
  for (const item of [...transientItems.value].reverse()) {
    if (item.kind === 'draft_card' && item.draft.previewUrl) return item.draft.previewUrl
  }
  const saved = [...toolCalls.value]
    .filter(t => t.tool_name === 'save_design_draft' && t.status === 'success')
    .sort((a, b) => tsOf(b.ended_at || b.started_at) - tsOf(a.ended_at || a.started_at))[0]
  if (!saved?.result_text) return ''
  try {
    const parsed = JSON.parse(saved.result_text)
    return parsed?.preview_url || ''
  } catch {
    return ''
  }
})
...
watch(
  ...
  ([open, n]) => {
    if (open && n > 0 && !activeArtifactContent.value && designDocumentArtifact.value) {
      loadArtifact(designDocumentArtifact.value)
    }
  },
)
...
async function loadArtifact(a: AIChatArtifact) {
  ...
  try {
    ...
    activeArtifactContent.value = detail.content || ''
    const versions = await aiChatApi.listArtifactVersions(currentSession.value!.id, a.filename)
    activeArtifactVersions.value = versions.versions || []
  } catch (e) {
...
// 用户选「新建」→ 留在 AIChat 主线，让 agent 走 save_design_draft → promote_draft_to_app
async function handleChooseNew() {
  chooseDialogVisible.value = false
  ElMessage.success('已发送，正在创建设计文档...')
  inputText.value = `请基于《${chooseDialogFilename.value}》保存设计文档并创建应用，完成后告诉我 app_id 和 apaas_app_id。`
  await nextTick()
  onSend()
}
...
function handleChooseUpdate(appId: number, appName: string) {
  ...
  chooseDialogVisible.value = false
  inputText.value = `请基于《${chooseDialogFilename.value}》更新应用 app_id=${appId}。先 get_application 拿 current_draft_id，再用 patch_design_draft 生成新版设计文档和预览；等我确认后再 apply_draft_to_live_app。`
  nextTick().then(() => onSend())
}
...
// 在 chatbox 注入一条"生成应用"命令，让 agent 调 save_design_draft/promote_draft_to_app 工具
...
async function sendGenerateAppMessage() {
  ...
  inputText.value = `请基于《${activeArtifactName.value}》保存设计文档并创建应用，完成后告诉我 app_id 和 apaas_app_id。`
  await nextTick()
  onSend()
}
...
async function sendArtifactToBuilder() {
  ...
  if (currentSession.value) {
    ...
    if (cachedId) {
      ...
      await router.push({ path: '/ai-chat', query: { app_id: String(cachedId) } })
      return
    }
  }
...
.draft-saved-card {
  ...
  padding: 13px 16px;
  border-radius: 10px;
  background: color-mix(in srgb, var(--ac-brand) 7%, transparent);
  border: 1px solid color-mix(in srgb, var(--ac-brand) 32%, transparent);
  max-width: 620px;
  width: 100%;
}
.draft-card-icon {
...
.draft-card-action {
  flex-shrink: 0;
  appearance: none;
  border: 1px solid color-mix(in srgb, var(--ac-brand) 45%, transparent);
  background: color-mix(in srgb, var(--ac-brand) 12%, transparent);
  color: var(--ac-brand);
  height: 32px;
  padding: 0 12px;
...
.draft-card-action:hover {
  background: color-mix(in srgb, var(--ac-brand) 18%, transparent);
}
.draft-card-action.secondary {
  background: var(--ac-bg);
  color: var(--ac-text);
  border-color: var(--ac-border);
}
.draft-card-action.secondary:hover {
