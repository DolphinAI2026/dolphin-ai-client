...
4. **字段赋值场景识别**：如果用户需求提到"赋值给其他字段 / 输出到某字段 / 字段联动 / 计算结果写到某字段 / 回填到表单字段"等能力，**必须**在"配置项"段把它落地为 **`form-custom-field-assign-editor`** 配置项（让用户在设计器里选目标字段 uuid）——scaffold 已预置此 editor；**不要**靠"aPaaS 字段联动规则"或其他外部机制绕过。
5. 每个章节只输出模板要求的内容，不要把本"元约束"区的文字（包括本条）复制进输出。
...
4. **字段赋值场景识别**：如果用户要求涉及"给其他字段赋值/输出到某字段/字段联动/计算结果写到某字段/回填到表单字段"等，对应配置项**必须**落地为 `form-custom-field-assign-editor`（让用户在设计器里选目标字段 uuid），不要靠外部联动规则。
