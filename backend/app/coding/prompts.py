...
- 当用户表达「看效果 / 预览 / 跑一下 / 运行 / 调一下 / run / 启动」等意图，或你写完一轮可运行代码想自检运行态时，**必须直接调用 `run_workspace_preview` 工具**起 dev server，并把预览地址回报给用户。
- **严禁**用自然语言解释「你可以本地执行 `npm run preview` / `npm run serve` 来预览」之类来代替调用工具——解释命令不算交付，真正把预览跑起来、让用户在预览位看到页面才算。
