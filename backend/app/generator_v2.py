...
# 权限原子操作收敛到 operations 层。_parse_permission_ops canonical = step_executor 超集版；
# _build_permission_groups_for_form_config 3-6 收口 canonical = gen_v2 9字段superset + step
# operation permissionRange（平台确认 honor 9 个 advanced 字段，见 operations/permissions.py）。
from app.operations.permissions import (  # noqa: F401
    _parse_permission_ops,
    _permission_object_for_form_config,
...
# _permission_object_for_form_config / _build_permission_groups_for_form_config 已收口到
# app.operations.permissions(3-6,见上方 import)。canonical = 本 gen_v2 9字段superset +
# step_executor 的 operation permissionRange。两条 0-1 路径共享单一对象。
...
# canonical = gen_v2 编排(超集 fetch + all_model_codes)+ step 的 form_id 早返守卫。
