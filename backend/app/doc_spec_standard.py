...
文档必须严格按照以下 6 个章节的顺序输出。每个章节的表头列名必须与下面给出的列名**逐字一致**，不要新增、删除或改字。

## ⚠️ 标题层级硬约束（最常见错误，违反会被 Builder 直接拒绝）

**6 个章节的标题必须用 `## ` 二级标题，不要用 `# ` 一级或 `### ` 三级**。doc_pipeline 章节切分器只认 `## ` 二级标题，标题层级错了整篇文档会被判为 0 分。
...
子章节（如 4.1 / 5.2）以及字典子章节（如 3.1）一律用 `### ` 三级。
