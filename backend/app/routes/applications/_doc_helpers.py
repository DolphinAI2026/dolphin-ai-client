...
- **字段业务含义必须写清楚**：客户、供应商、员工、部门、项目、产品等来自其他业务对象的字段，不是普通文本；在字段 description 中写明 `数据选择: 目标表.显示字段`
- **固定枚举值必须给选项来源**：状态、类型、等级、是否等字段，在字段上补 `options` 数组（如 ["草稿","已提交","已审批"]），后续会派生数据字典并绑定下拉
- **不要把选择类字段当单行输入**：只有名称、备注、编码等自由文本才用 VARCHAR + 普通描述；业务对象选择、固定枚举、人员/部门选择要在 description/options 中表达来源
...
- **权限要覆盖页面 + 数据范围**：每张主表至少给业务角色和全部员工/发起人明确新增、查看、编辑、审批、导出等操作范围，避免生成后用户看不到或保存时报无权限
