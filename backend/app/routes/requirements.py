...
唯一数据源：dolphin agent 81 调 mcp_server.submit_design_doc 工具时把 md
...
> 历史：曾用 5s 轮询 + dolphin chat history API 抽 markdown code block 作为 fallback
> （当时 dolphin agent 工具关联只识别 9 个工具调不到 submit_design_doc）。该
> workaround 已删除。如需排查可看 git blame `_try_extract_md_from_dolphin`。
"""
...
async def get_latest_requirements_doc(
    ...
    返回 has_doc=False 时前端显示空态（提醒用户去 dolphin chat 让 agent 重发一次）。
    """
