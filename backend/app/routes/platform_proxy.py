...
def _inject_sso_script(html: str, vuex_state: str) -> str:
    ...
    # 2026-05-25 v3: widget-panel 折叠 — display:none 路线 (放弃 width 0 战 calc min-width)
    # 按钮重设计: 极简竖条 handle, 4px 宽贴 iframe 左缘, 鼠标 hover 才显形
    panel_style = (
        "\n<style id='apaas-builder-panel-toggle'>\n"
        # 折叠态 — display:none 直接抹掉 panel, flex 兄弟自然填满, 无 width 战
        "body.apaas-builder-panel-collapsed .widget-panel,\n"
        "body.apaas-builder-panel-collapsed [class*='widget-panel'] {\n"
        "  display: none !important;\n"
        "}\n"
        # 按钮容器 — 极简竖条 hover 触发区, 默认几乎不可见
        ...
        "  outline: none;\n"
        "}\n"
        # 内部 chevron 圆点 — 默认半透明灰, hover 时明显
        "#apaas-builder-panel-toggle-btn .ams-chev {\n"
        "  width: 14px;\n"
        "  height: 36px;\n"
        ...
        "  user-select: none;\n"
        "}\n"
        "#apaas-builder-panel-toggle-btn:hover .ams-chev {\n"
        "  background: rgba(124, 58, 237, 0.85);\n"
        "  width: 18px;\n"
        "}\n"
        "#apaas-builder-panel-toggle-btn:active .ams-chev {\n"
    ...
    panel_script = (
        ...
        "    chev.className = 'ams-chev';\n"
        "    chev.textContent = '\\u2039';\n"
        "    btn.appendChild(chev);\n"
        "    btn.addEventListener('click', function(e){\n"
        "      e.preventDefault();\n"
        "      e.stopPropagation();\n"
        "      var collapsed = document.body.classList.toggle('apaas-builder-panel-collapsed');\n"
        ...
        "    makeBtn();\n"
        "  }\n"
        # SPA route 切 widget-panel 重渲后, 按钮不在新 body 里 → 兜底 re-inject
