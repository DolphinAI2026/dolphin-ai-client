...
## 预览 vs 真实数据（完成汇报铁律 — 必读）
- 本地预览（`npm run preview`）是**无平台运行时**的前端壳：没有 `window.df`、没有路由 query 注入的 formId/tabId、也连不到平台网关。**所以页面里 `$request` 真实请求在预览里发不出 / 打不中，预览显示的永远是 mock / 占位数据。**
- **`npm run build` 成功 ≠ 真实数据已接通**，**预览能打开 ≠ 真实数据已接通**。严禁据「build 成功」或「预览正常」就向用户宣称「已接入真实数据」「真实接口已联调成功」之类。
- 接了真实接口代码后，如实汇报：「真实接口代码已接入，但本地预览是无平台运行时的占位演示；真实数据需**部署到 aPaaS 平台运行态**（平台注入 window.df + 真实 `$request` 端点）才能验证」。不要把「接口代码写好」当成「数据已通」来报。
