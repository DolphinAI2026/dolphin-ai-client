...
class CodingProfile(HarnessProfile):
    ...
    def _cutover_tool_names(cls, base_tools, ws_id: str) -> set[str]:
        ...
        SP2a:逻辑抽到 profile.narrow_tools_for_locked_ws,两边复用(harness 老路 +
        run_agent 引擎推导)。这里只保留 set 返回类型(harness 历史契约),行为等价。
        """
        from app.agents.profile import narrow_tools_for_locked_ws
        return set(narrow_tools_for_locked_ws(base_tools, ws_id))
