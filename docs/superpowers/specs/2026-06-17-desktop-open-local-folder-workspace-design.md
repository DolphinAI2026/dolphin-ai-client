...
2. **无独立删除工具（实测，不做软删除）**：coding 7 个 base 工具（read/write/edit/run_command/glob/grep/start_serve）**无 delete 工具**；`tools.py:358` 的 `unlink()` 是 `_edit_file` 改名时删旧路径的内部清理，非 agent 删用户文件。agent 删文件只能经 run_command（本轮不约束）。文件工具真正破坏面 = `write_file` 覆盖已有文件（内容丢失），已有「补丁守卫」软警告（>50% 重写，`tools.py`）。**恢复网主要靠用户自己的 git**（打开时风险确认引导选 git/备份目录）——与 Claude Code/Cursor 一致（不做 app 层软删/备份：无 delete 工具可挂、且与 git 冗余）。故本轮不引入软删除回收站。
...
- Part D：`_resolve_safe` 对 external root 拒 `..`/越界（已有逻辑回归 + external root 用例）；敏感目录（`/`、家目录根、`/System` 等）注册被拒（`_is_sensitive_dir` 单测）；打开流程含一次性风险确认（前端）。无软删除（无 delete 工具，不测）。
...
- run_command 开口 shell 是已知未闭合风险（本轮不约束）：用户真实文件夹下 agent 经 run_command 仍能触达机器任意路径，且能删/改文件（绕过文件工具的路径牢笼）。本轮主要缓解 = 打开时风险确认 + 引导用户选 git/备份目录（git 即恢复网）。真闭合需 P2 沙箱。
