// 平台运行时注入的验证器 — 构建时通过 externals 或运行时获取
const WidgetRequiredValidator = (window.Vue && window.Vue.FormEngine && window.Vue.FormEngine.WidgetRequiredValidator) ||
  function(message) {
...
const WidgetRegexValidator = (window.Vue && window.Vue.FormEngine && window.Vue.FormEngine.WidgetRegexValidator) ||
...
const debounce = (window._ && window._.debounce) || function(fn, wait) {
  let timer
  const debounced = function(...args) { clearTimeout(timer); timer = setTimeout(() => fn.apply(this, args), wait) }
  debounced.cancel = function() { clearTimeout(timer) }
  return debounced
}
const XEventBus = (window.APaaSSDK && window.APaaSSDK.context && window.APaaSSDK.context.XEventBus) || { emit() {}, on() {}, off() {} }
const debounceWaitTime = 150
const AbilityControl = (window.Vue && window.Vue.FormEngine && window.Vue.FormEngine.AbilityControl) || { formatFiledValue() { return '' }, TITLE_DESCRIPTION_FORM_FIELD: '', FORM_VALIDATOR_FORM_FIELD: '' }
