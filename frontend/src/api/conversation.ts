...
export const conversationApi = {
  ...
  /** 切换对话的 agent_type（如 requirements → builder） */
  updateAgentType(id: number, agent_type: string) {
    return request.patch<any, Conversation>(`/conversations/${id}/agent-type`, { agent_type })
  },
...
export async function convertConfig(docResult: any): Promise<any> {
  const resp = await request.post<any, { config: any }>('/applications/convert-config', { doc_result: docResult })
  return resp.config
}
