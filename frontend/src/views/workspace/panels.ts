...
export function registerPhase1Panels(): void {
  ...
  // 预览面板(起 dev server + iframe 看页面): 仅 workspace 绑定可用
  registerPanel({ id: 'preview', label: '预览', icon: 'monitor', group: 'context',
    availableWhen: (b) => b.kind === 'workspace', component: defineAsyncComponent(() => import('./panels/PreviewPanel.vue')) })
