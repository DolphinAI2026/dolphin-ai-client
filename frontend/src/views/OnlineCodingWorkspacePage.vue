<template>
  <BuilderFrame :breadcrumbs="[{ label: 'Vibe Coding' }]">
    ...
    <main class="oc-page builder-page is-ide-view">
      ...
      <template v-else>
        <section class="oc-ide-toolbar">
          ...
          <div class="oc-ide-actions">
            <span class="oc-runtime-pill" :class="runtimeStatusClass">
              {{ runtimeStatusLabel }}
            </span>
            <button class="oc-secondary" type="button" :disabled="runtimeBusy" @click="refreshPreviewRuntimeStatus(true)">
            ...
            >
              ...
              <span>{{ runtimeBusy ? '启动中' : '启动预览' }}</span>
            </button>
        ...
        <section class="oc-runtime-panel">
...
async function loadFromRoute() {
  ...
  try {
    ...
    if (isWorkspaceReady(loaded)) {
      await prepareStudio()
      await refreshPreviewRuntimeStatus(false)
      await openIde(false)
    } else {
      resetRuntimeState()
